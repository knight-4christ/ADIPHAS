...
$null = Start-Process -FilePath "$PSScriptRoot\myenv\Scripts\python.exe" `
...
if (-not $healthy) {
    ...
    Write-Host "      Check: logs\adiphas.log for details" -ForegroundColor Red
}
...
Write-Host "  Logs: logs\adiphas.log (tailed below)" -ForegroundColor Gray
...
Get-Content -Path "$PSScriptRoot\logs\adiphas.log" -Wait -Tail 20
