...
def render(is_overlay=False):
    ...
    t_act1, t_act2, t_act3, t_act4 = st.columns([3, 1, 1, 1])
    with t_act1:
        st.caption(f"🧵 Mode: `{active_thread['mode']}` | Created: {active_thread['timestamp'].strftime('%H:%M')}")
    with t_act2:
        # Download conversation
        if active_thread["messages"]:
            conv_text = f"ADIPHAS Advisory Chat\nThread: {active_thread['name']}\nDate: {datetime.now().strftime('%B %d, %Y')}\n{'=' * 50}\n\n"
            for msg in active_thread["messages"]:
                role_label = "You" if msg["role"] == "user" else "ADIPHAS AI"
                conv_text += f"[{role_label}]\n{msg['content']}\n\n"
            conv_text += f"{'=' * 50}\nGenerated by ADIPHAS - Autonomous Disease Intelligence Platform"
            
            from .download_utils import _generate_pdf_bytes
            pdf_bytes = _generate_pdf_bytes(conv_text, title=f"Advisory Chat — {active_thread['name']}")
            if pdf_bytes:
                st.download_button(
                    "📄 PDF",
                    data=pdf_bytes,
                    file_name=f"adiphas_chat_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf",
                    mime="application/pdf",
                    use_container_width=True,
                    key=f"dl_chat_pdf_{st.session_state.active_thread_id}"
                )
    with t_act3:
        if active_thread["messages"]:
